  ...
  void _calculate() {
    ...
    if (type == 'acyk') {
      ...
      if (valB == null || valH == null) {
        showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text("Ýalňyşlyk"), content: const Text("Dogry san giriziň!"), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("OK"))]));
        return;
      }
      ...
      if (f == null || valV == null) {
        showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text("Ýalňyşlyk"), content: const Text("Diametr ýa-da tizlik nädogry!"), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("OK"))]));
        return;
      }
      q = f * valV;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            const Text("Ýap Gidrawlika Hasaby", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            ElevatedButton(
              onPressed: () => setState(() => type = type == 'acyk' ? 'yapyk' : 'acyk'),
              style: ElevatedButton.styleFrom(backgroundColor: type == 'acyk' ? Colors.blue : Colors.deepPurple, foregroundColor: Colors.white),
            ...
            if (type == 'acyk') ...[
              ...
              TextField(controller: _hController, decoration: const InputDecoration(labelText: "Suw gatlagy (H)"), keyboardType: TextInputType.number),
            ] else ...[
              TextField(controller: _dController, decoration: const InputDecoration(labelText: "Diametr (mm: 300-1000)"), keyboardType: TextInputType.number),
              TextField(controller: _vController, decoration: const InputDecoration(labelText: "Tizlik (m/s)"), keyboardType: TextInputType.number),
            ],
            const SizedBox(height: 20),
            ElevatedButton(
            ...
            const SizedBox(height: 20),
            Text("Netije: $_result m³/s", style: const TextStyle(fontSize: 20, color: Color(0xFF1e3a8a), fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
